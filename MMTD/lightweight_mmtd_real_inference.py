...
from transformers import DistilBertTokenizerFast, ViTFeatureExtractor
import numpy as np
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="LightWeightMMTD 실제 데이터 실험")
    parser.add_argument('--input', type=str, required=True, help='입력 JSON 파일 경로')
    parser.add_argument('--checkpoint', type=str, required=True, help='LightWeightMMTD 체크포인트(.pth) 경로')
    parser.add_argument('--output', type=str, default='lightweight_mmtd_real_inference_result.json', help='결과 저장 파일')
    ...
    # 모델 로드
    print("[INFO] LightWeightMMTD 모델 로딩...")
    ...
    # 실제 모델 추론
    print("[INFO] LightWeightMMTD 모델로 추론...")
    with torch.no_grad():
        outputs = model(
            input_ids=input_ids,
