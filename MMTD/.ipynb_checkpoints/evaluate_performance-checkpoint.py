...
def load_model_checkpoint(checkpoint_path, device='cpu'):
    ...
    # 훈련 시 사용한 프리트레인된 BERT 및 BEiT 모델 이름을 정확히 지정합니다.
    # 오류 메시지 상 텍스트 인코더는 'bert-base-multilingual-cased'일 가능성이 높습니다.
    # BEiT는 'microsoft/dit-base'를 콜레이터에서 사용하므로 이를 따르는 것이 합리적입니다.
    BERT_MODEL_NAME = "bert-base-multilingual-cased"
    BEIT_MODEL_NAME = "microsoft/dit-base" 

    # MMTD 모델을 프리트레인된 가중치와 함께 초기화합니다.
    model = MMTD(
        bert_pretrain_weight=BERT_MODEL_NAME,
        beit_pretrain_weight=BEIT_MODEL_NAME
    )
    ...
    # MMTD 클래스의 __init__에서 BertForSequenceClassification을 로드할 때
    # "cls.predictions.bias"와 같은 몇몇 키가 state_dict에 없을 수 있습니다.
    # 또한, "classifier" 레이어의 이름이 다를 수 있습니다.
    # 그러나 'text_encoder'와 'image_encoder'의 메인 가중치는 로드되어야 합니다.
    # 여기서는 strict=False를 유지하여 일치하지 않는 키는 무시하고 로드하도록 합니다.
