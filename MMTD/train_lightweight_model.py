...
class DynamicCollator:
    def __init__(self, text_model_name, image_model_name):
        ...
        if "mobilevit" in image_model_name:
            self.feature_extractor = MobileViTImageProcessor.from_pretrained(
                image_model_name
            )
...
class DataSplitter:
    ...
    def get_fold_data(self, test_fold_idx):
        ...
        train_data = pd.concat(
            [self.folds[i] for i in range(self.k_fold) if i != test_fold_idx],
            ignore_index=True,
        )
...
def train_model(
    ...
    num_epochs=3,
    learning_rate=2e-5,
    save_path="lightweight_checkpoints",
):
    """모델 훈련"""
    model.to(device)
...
def plot_training_history(history, save_path="lightweight_checkpoints"):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_path, "training_history.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
...
experiment_configs = {
    ...
    "mobilebert_mobilevit": {
        ...
        "collator_class": lambda: DynamicCollator(
            "google/mobilebert-uncased", "apple/mobilevit-small"
        ),
    ...
    "mobilebert_deit": {
        "model_class": GeneralizedMMTD,
        "collator_class": lambda: DynamicCollator(
            "google/mobilebert-uncased", "facebook/deit-base-patch16-224"
        ),
        "text_encoder_cls": MobileBertForSequenceClassification,
    ...
    "distilbert_mobilevit": {
        "model_class": GeneralizedMMTD,
        "collator_class": lambda: DynamicCollator(
            "distilbert-base-multilingual-cased", "apple/mobilevit-small"
        ),
    ...
    "distilbert_deit": {
        "model_class": GeneralizedMMTD,
        "collator_class": lambda: DynamicCollator(
            "distilbert-base-multilingual-cased", "facebook/deit-base-patch16-224"
        ),
        "text_encoder_cls": DistilBertForSequenceClassification,
        "image_encoder_cls": AutoModelForImageClassification,
        "text_encoder_name": "distilbert-base-multilingual-cased",
    ...
    "tinybert_vit-tiny": {
        "model_class": GeneralizedMMTD,
        "collator_class": lambda: DynamicCollator(
            "huawei-noah/TinyBERT_General_4L_312D", "WinKawaks/vit-tiny-patch16-224"
        ),
    ...
    "mobilebert_beit": {
        ...
        "collator_class": lambda: DynamicCollator(
            "google/mobilebert-uncased", "microsoft/beit-base-patch16-224"
        ),
        "text_encoder_cls": MobileBertForSequenceClassification,
        "text_encoder_name": "google/mobilebert-uncased",
    ...
    "distilbert_beit": {
        "model_class": HybridMMTDTextTrainable,
        "collator_class": lambda: DynamicCollator(
            "distilbert-base-multilingual-cased", "microsoft/beit-base-patch16-224"
        ),
        "text_encoder_cls": DistilBertForSequenceClassification,
        "text_encoder_name": "distilbert-base-multilingual-cased",
    ...
    "tinybert_beit": {
        "model_class": HybridMMTDTextTrainable,
        "collator_class": lambda: DynamicCollator(
            "huawei-noah/TinyBERT_General_4L_312D", "microsoft/beit-base-patch16-224"
        ),
        "text_encoder_cls": AutoModelForSequenceClassification,
        "text_encoder_name": "huawei-noah/TinyBERT_General_4L_312D",
...
def train_single_fold(
    ...
    report = classification_report(
        labels, predictions, target_names=["ham", "spam"], output_dict=True
    )
